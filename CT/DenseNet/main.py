...
from model import Valve2DDenseNetImageModel
...
def calculate_gradcam(model,image,device,target_class=1):
    model=model.to(device); model.eval(); image=image.unsqueeze(0).to(device); image.requires_grad_(True); activations={}; gradients={}; target_layer=model.gradcam_target_layer()
    fh=target_layer.register_forward_hook(lambda m,i,o:activations.__setitem__("value",o)); bh=target_layer.register_full_backward_hook(lambda m,gi,go:gradients.__setitem__("value",go[0]))
    try:
        model.zero_grad(set_to_none=True); logits=model(image); logits[0,target_class].backward(); a=activations["value"][0]; g=gradients["value"][0]; w=g.mean(dim=(1,2)); cam=torch.relu(torch.sum(w[:,None,None]*a,dim=0)); cam=normalize_cam(cam.detach().cpu().numpy())
    finally:
        fh.remove(); bh.remove()
    return cam
...
def main(config):
    ...
    for fold_index in range(n_folds):
        ...
        if resolved["mode"]=="train":
            ...
            best_model=Valve2DDenseNetImageModel.load_from_checkpoint(best_path,map_location=device)
        else:
            logger=None; trainer=None; best_path=find_existing_checkpoint(resolved["checkpoint_root"],fold_number); print(f"Loading checkpoint: {best_path}"); best_model=Valve2DDenseNetImageModel.load_from_checkpoint(str(best_path),map_location=device)
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description="CT-only DenseNet predefined-fold experiment."); parser.add_argument("--config_file",type=str,default="config.json"); args=parser.parse_args(); config_path=Path(args.config_file)
