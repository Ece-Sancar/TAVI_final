...
class Valve2DDenseNetModel(
    ...
    def __init__(
        ...
        self._partial_unfreeze_applied = (
            False
        )
    ...
    def _freeze_backbone_batch_norm(
        self,
    ):
        for module in (
            self.densenet
    ...
    def _unfreeze_requested_backbone_part(
        self,
    ):
        n_blocks = int(
            self.hparams
        ...
        if n_blocks > 4:
            raise ValueError(
                "DenseNet121 has four dense "
                "blocks; requested more than four."
            )
        ...
        print(
            f"Unfroze DenseNet modules "
            f"{names}; "
            f"trainable backbone "
    ...
    def extract_image_features(
        ...
        # Part of torchvision DenseNet's
    ...
    def forward(
        ...
        if not torch.isfinite(
            ...
            raise FloatingPointError(
                "Non-finite logits were produced."
            )
    ...
    def gradcam_target_layer(
        self,
    ):
        """
        Explicit target used by the Grad-CAM code.

        This is the last convolutional feature stage
        immediately before DenseNet's final activation
