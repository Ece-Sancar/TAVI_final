...
"""
Standalone tabular MLP using the exact tabular feature extractor from the
...
def predict_probabilities(
    ...
    expected_width = int(
        model.tabular_net[0].in_features
    )
    actual_width = int(
        X.shape[1]
    )
    ...
    if actual_width != expected_width:
        raise ValueError(
            "Model/preprocessing input mismatch: "
            f"checkpoint expects {expected_width} inputs, "
            f"but X has {actual_width}. "
            "Use the tabular_preprocessor.json saved with this checkpoint "
            "and make sure transform_tabular respects its missingness_mask flag."
        )
...
def calculate_paired_permutation_importance(
    ...
    """
    Permutation importance supporting both no-mask and mask models.
    """
    ...
    elif X_test.shape[1] == 2 * number_of_features:
        uses_missingness_mask = True
    else:
        raise ValueError(
            "Unexpected model input width for permutation importance. "
            f"Original features={number_of_features}, "
            f"X_test width={X_test.shape[1]}. "
            "Expected F or 2F."
        )
    ...
    for feature_index in range(number_of_features):
        ...
        for repeat_number in range(number_of_repeats):
            random_generator = np.random.RandomState(
                seed + feature_index * 1009 + repeat_number
            )
            ...
            permuted_X[:, feature_index] = X_test[
                permutation_indices,
                feature_index,
            ]

            if uses_missingness_mask:
                mask_index = feature_index + number_of_features
                permuted_X[:, mask_index] = X_test[
                    permutation_indices,
                    mask_index,
                ]
        ...
        if number_of_repeats > 1:
            repeat_standard_deviations[feature_index] = float(
                np.std(repeat_losses, ddof=1)
            )
...
def main() -> None:
    ...
    for fold_number in range(
        ...
        if resolved["mode"] == "train":
            ...
            model = (
                TabularMLP.load_from_checkpoint(
                    best_checkpoint,
                    class_weights=(
                        class_weights
                    ),
    ...
    summary = {
        ...
        "architecture": (
            "Input (checkpoint-defined preprocessing) "
            "-> 128 -> 64 -> 32 -> 2 logits"
        ),
        "preprocessing": (
            "For pretrained mode, use each checkpoint fold's saved "
            "tabular_preprocessor.json exactly, including whether "
            "missingness masks were used."
        ),
        "feature_importance": (
    ...
    with (
        ...
        handle.write(
            "Pretrained mode uses the preprocessing state saved with each checkpoint.\n"
        )

        handle.write(
            "This includes feature order, medians, scaler, and whether "
            "missingness masks were used.\n\n"
        )
