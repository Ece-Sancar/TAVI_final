...
"""
Plain tabular MLP WITHOUT missingness masks + Optuna hyperparameter tuning.
...
def parse_mlp_architecture(
    architecture: str,
) -> List[int]:
    """
    Convert strings such as "64-32-16" into [64, 32, 16].
    """
    try:
        hidden_dims = [
            int(value)
        ...
        raise ValueError(
            f"Invalid MLP architecture: {architecture!r}"
        ) from error

    if len(hidden_dims) < 2:
        raise ValueError(
            "MLP architecture must contain at least two hidden layers."
        )

    if any(dim <= 0 for dim in hidden_dims):
        raise ValueError(
            f"MLP architecture contains a non-positive dimension: "
            f"{architecture!r}"
        )
...
def build_model(
    ...
    hidden_dims = parse_mlp_architecture(
        parameters[
            "mlp_architecture"
        ]
    )
...
def sample_parameters(
    ...
    """
    Tune a compact set of plain-MLP architectures plus optimization settings.

    The exact requested architecture, 64 -> 32 -> 16, is included as one
    categorical option. Other options vary both depth and width while keeping
    the model family strictly as a feed-forward MLP.
    """
    space = config[
        "optuna"
    ][
